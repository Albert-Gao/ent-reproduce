...
func (tq *TenantQuery) QueryMembers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, tenant.MembersTable, tenant.MembersPrimaryKey...),
		)
...
// QueryMemberProfiles chains the current query on the "memberProfiles" edge.
func (tq *TenantQuery) QueryMemberProfiles() *ProfileQuery {
	query := &ProfileQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, tenant.MemberProfilesTable, tenant.MemberProfilesColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tq *TenantQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (tq *TenantQuery) Clone() *TenantQuery {
	...
	return &TenantQuery{
		...
		withMemberProfiles: tq.withMemberProfiles.Clone(),
...
func (tq *TenantQuery) WithMembers(opts ...func(*UserQuery)) *TenantQuery {
	query := &UserQuery{config: tq.config}
...
// WithMemberProfiles tells the query-builder to eager-load the nodes that are connected to
// the "memberProfiles" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithMemberProfiles(opts ...func(*ProfileQuery)) *TenantQuery {
	query := &ProfileQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withMemberProfiles = query
	return tq
}
