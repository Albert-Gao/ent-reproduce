...
func HasMembersWith(preds ...predicate.User) predicate.Tenant {
	return predicate.Tenant(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasMemberProfiles applies the HasEdge predicate on the "memberProfiles" edge.
func HasMemberProfiles() predicate.Tenant {
	return predicate.Tenant(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MemberProfilesTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, MemberProfilesTable, MemberProfilesColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasMemberProfilesWith applies the HasEdge predicate on the "memberProfiles" edge with a given conditions (other predicates).
func HasMemberProfilesWith(preds ...predicate.Profile) predicate.Tenant {
	return predicate.Tenant(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MemberProfilesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, MemberProfilesTable, MemberProfilesColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
