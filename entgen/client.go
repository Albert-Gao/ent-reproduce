...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	if _, ok := c.driver.(*txDriver); ok {
		return nil, errors.New("entgen: cannot start a transaction within a transaction")
	}
...
// QueryTenant queries the tenant edge of a Profile.
func (c *ProfileClient) QueryTenant(pr *Profile) *TenantQuery {
	...
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, profile.TenantTable, profile.TenantColumn),
		)
		fromV = sqlgraph.Neighbors(pr.driver.Dialect(), step)
		return fromV, nil
	}
...
func (c *TenantClient) QueryMembers(t *Tenant) *UserQuery {
	query := &UserQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
...
// QueryMemberProfiles queries the memberProfiles edge of a Tenant.
func (c *TenantClient) QueryMemberProfiles(t *Tenant) *ProfileQuery {
