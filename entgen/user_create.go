...
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "create_time", err: errors.New(`entgen: missing required field "User.create_time"`)}
	}
	if _, ok := uc.mutation.UpdateTime(); !ok {
		return &ValidationError{Name: "update_time", err: errors.New(`entgen: missing required field "User.update_time"`)}
	}
	if len(uc.mutation.TenantsIDs()) == 0 {
		return &ValidationError{Name: "tenants", err: errors.New(`entgen: missing required edge "User.tenants"`)}
	}
	return nil
}
